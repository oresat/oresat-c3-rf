...
uint8_t __xdata radio_lcd_display[] = {"found AX5043\n"};
...
static __reentrantb void update_timeanchor(void) __reentrant
{
    ...
    axradio_timeanchor.radiotimer = radio_read24(AX5043_REG_TIMER2);
    exit_critical(crit);
}
...
static __reentrantb uint8_t ax5043_init_registers_common(void) __reentrant
{
	...
	if (rng & 0x80)
		radio_write8(AX5043_REG_PLLVCOI, rng);
...
static __reentrantb void receive_isr(void) __reentrant
{
	...
	uint8_t i;
	uint8_t len = radio_read8(AX5043_REG_RADIOEVENTREQ0); // clear request so interrupt does not fire again. sync_rx enables interrupt on radio state changed in order to wake up on SDF detected

	uint8_t radioStateTemp = radio_read8(AX5043_REG_RADIOSTATE);
...
#ifdef DBGPKT
	if (len & 0x0C) {
		len = radio_read8(AX5043_REG_RADIOSTATE);
		radio_write8(AX5043_REG_PINFUNCDATA, (len & 0x01));
		radio_write8(AX5043_REG_PINFUNCPWRAMP, ((len >> 1) & 0x01));
	}
#endif
    ...
    while (radio_read8(AX5043_REG_IRQREQUEST0) & 0x01) {    // while fifo not empty
		...
		case AX5043_FIFOCMD_DATA:
			...
			if(axradio_mode == AXRADIO_MODE_WOR_RECEIVE || axradio_mode == AXRADIO_MODE_WOR_ACK_RECEIVE) {
				f30_saved = radio_read8(AX5043_REG_0xF30);
				f31_saved = radio_read8(AX5043_REG_0xF31);
				f32_saved = radio_read8(AX5043_REG_0xF32);
				f33_saved = radio_read8(AX5043_REG_0xF33);
			}
			...
			radio_write8(AX5043_REG_IRQMASK0, (radio_read8(AX5043_REG_IRQMASK0) & (uint8_t)~0x01)); // disable FIFO not empty irq
			...
			if (AXRADIO_MODE_IS_STREAM_RECEIVE(axradio_mode)) {
				...
				if (axradio_phy_innerfreqloop) {
                    axradio_cb_receive.st.rx.phy.offset.o = axradio_conv_freq_fromreg(signextend16(radio_read16(AX5043_REG_TRKFREQ1)));
				} else {
		...
		case AX5043_FIFOCMD_RFFREQOFFS:
			if (axradio_phy_innerfreqloop || len != 3)
				goto dropchunk;
			i = radio_read8(AX5043_REG_FIFODATA);
			i &= 0x0F;
			i |= 1 + (uint8_t)~(i & 0x08);
			axradio_cb_receive.st.rx.phy.offset.b.b3 = ((int8_t)i) >> 8;
			axradio_cb_receive.st.rx.phy.offset.b.b2 = i;
			axradio_cb_receive.st.rx.phy.offset.b.b1 = radio_read8(AX5043_REG_FIFODATA);
			axradio_cb_receive.st.rx.phy.offset.b.b0 = radio_read8(AX5043_REG_FIFODATA);
			break;
		...
		case AX5043_FIFOCMD_RSSI:
			if (len != 1)
				goto dropchunk;
			{
				int8_t r = radio_read8(AX5043_REG_FIFODATA);
				axradio_cb_receive.st.rx.phy.rssi = r - (int16_t)axradio_phy_rssioffset;
			}
			break;
		...
		case AX5043_FIFOCMD_TIMER:
			...
			axradio_cb_receive.st.time.b.b2 = radio_read8(AX5043_REG_FIFODATA);
			axradio_cb_receive.st.time.b.b1 = radio_read8(AX5043_REG_FIFODATA);
			axradio_cb_receive.st.time.b.b0 = radio_read8(AX5043_REG_FIFODATA);
			break;
		...
		dropchunk:
			...
			i = len;
			do {
				radio_read8(AX5043_REG_FIFODATA);	// purge FIFO
			}
			while (--i);
			break;
		} // end switch(fifo_cmd)
...
{
	for (;;) {
		uint8_t cnt = radio_read8(AX5043_REG_FIFOFREE0);
		if (radio_read8(AX5043_REG_FIFOFREE1))
			cnt = 0xff;
		...
		case trxstate_tx_longpreamble:
			...
			cnt <<= 5;
			radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_REPEATDATA | (3 << 5)));
			radio_write8(AX5043_REG_FIFODATA, axradio_phy_preamble_flags);
			radio_write8(AX5043_REG_FIFODATA, cnt);
			radio_write8(AX5043_REG_FIFODATA, axradio_phy_preamble_byte);
			break;
		...
		shortpreamble:
			if (!axradio_txbuffer_cnt) {
				...
				if (axradio_phy_preamble_appendbits) {
					uint8_t byte;
					radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_DATA | (2 << 5)));
					radio_write8(AX5043_REG_FIFODATA, 0x1C);
					byte = axradio_phy_preamble_appendpattern;
					if (radio_read8(AX5043_REG_PKTADDRCFG) & 0x80) {
					...
					radio_write8(AX5043_REG_FIFODATA, byte);
				}
#ifdef DBGPKT
                radio_write8(AX5043_REG_FIFODATA, AX5043_FIFOCMD_TXCTRL | (1 << 5));
				radio_write8(AX5043_REG_FIFODATA, 0x02); // set PA, state 0
#endif
                if ((radio_read8(AX5043_REG_FRAMING) & 0x0E) == 0x06 && axradio_framing_synclen) {
					...
					len_byte += 7;
					len_byte >>= 3;
					radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_DATA | ((len_byte + 1) << 5)));
					radio_write8(AX5043_REG_FIFODATA, axradio_framing_syncflags | i);
					for (i = 0; i < len_byte; ++i) {
						// better put a brace, it might prevent SDCC from optimizing away the assignement...
						radio_write8(AX5043_REG_FIFODATA, axradio_framing_syncword[i]);
					}
				}
#ifdef DBGPKT
				radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_TXCTRL | (1 << 5)));
				radio_write8(AX5043_REG_FIFODATA, 0x03); // set PA, state 1
#endif
		...
		case trxstate_tx_packet:
				...
				goto fifocommit;
			{
				...
				radio_write8(AX5043_REG_FIFODATA, (cnt + 1)); // write FIFO chunk length byte (length includes the flag byte, thus the +1)
				radio_write8(AX5043_REG_FIFODATA, flags);
  ...
  pktend:
	...
	radio_write8(AX5043_REG_RADIOEVENTMASK0, 0x01); // enable REVRDONE event
	radio_write8(AX5043_REG_IRQMASK0, 0x40); // enable radio controller irq
  fifocommit:
	radio_write8(AX5043_REG_FIFOSTAT, 4); // commit
}
...
#endif
{
	...
	case trxstate_tx_xtalwait:
		radio_read8(AX5043_REG_RADIOEVENTREQ0); // make sure REVRDONE bit is cleared, so it is a reliable indicator that the packet is out
		radio_write8(AX5043_REG_FIFOSTAT, 3); // clear FIFO data & flags (prevent transmitting anything left over in the FIFO, this has no effect if the FIFO is not powerered, in this case it is reset any way)
		radio_write8(AX5043_REG_IRQMASK1, 0x00);
		radio_write8(AX5043_REG_IRQMASK0, 0x08); // enable fifo free threshold
		axradio_trxstate = trxstate_tx_longpreamble;
#if 1
		if ((radio_read8(AX5043_REG_MODULATION) & 0x0F) == 9) { // 4-FSK
			radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_DATA | (7 << 5)));
			radio_write8(AX5043_REG_FIFODATA, 2);  // length (including flags)
			radio_write8(AX5043_REG_FIFODATA, 0x01);  // flag PKTSTART -> dibit sync
			radio_write8(AX5043_REG_FIFODATA, 0x11); // dummy byte for forcing dibit sync
		}
#endif
#ifdef DBGPKT
		radio_write8(AX5043_REG_FIFODATA, (AX5043_FIFOCMD_TXCTRL | (1 << 5)));
	...
	case trxstate_tx_waitdone:
		radio_read8(AX5043_REG_RADIOEVENTREQ0);
		radioStateTemp = radio_read8(AX5043_REG_RADIOSTATE);
		if (radioStateTemp != 0)
			break;
		radio_write8(AX5043_REG_RADIOEVENTMASK0, 0x00);
		switch (axradio_mode) {
	...
	case trxstate_txstream_xtalwait:
		if (radio_read8(AX5043_REG_IRQREQUEST1) & 0x01) {
			radio_write8(AX5043_REG_RADIOEVENTMASK0, 0x03); // enable PLL settled and done event
			radio_write8(AX5043_REG_IRQMASK1, 0x00);
			radio_write8(AX5043_REG_IRQMASK0, 0x40); // enable radio controller irq
			radio_write8(AX5043_REG_PWRMODE, AX5043_PWRSTATE_FULL_TX);
	...
	{
		uint8_t __autodata evt = radio_read8(AX5043_REG_RADIOEVENTREQ0);
		radio_write8(AX5043_REG_RADIOEVENTMASK0, 0x00);
		if (evt & 0x03)
			update_timeanchor();
		if (evt & 0x01) {
	...
	txstreamdatacb:
	if (radio_read8(AX5043_REG_IRQREQUEST0) & radio_read8(AX5043_REG_IRQMASK0) & 0x08) {
		radio_write8(AX5043_REG_IRQMASK0, (radio_read8(AX5043_REG_IRQMASK0) & (uint8_t)~0x08));
		update_timeanchor();
		wtimer_remove_callback(&axradio_cb_transmitdata.cb);
		axradio_cb_transmitdata.st.error = AXRADIO_ERR_NOERROR;
		axradio_cb_transmitdata.st.time.t = axradio_timeanchor.radiotimer;
...
__reentrantb void ax5043_receiver_on_continuous(void) __reentrant
{
	...
	if (axradio_mode == AXRADIO_MODE_ASYNC_RECEIVE ||
		...
		radio_write8(AX5043_REG_TMGRXPREAMBLE1, axradio_phy_tmgrxpreamble1_cont);
		radio_write8(AX5043_REG_PKTSTOREFLAGS, (radio_read8(AX5043_REG_PKTSTOREFLAGS) | 0x40));
	} else {
...
{
	radio_write8(AX5043_REG_PKTADDR0, axradio_localaddr.addr[0]);
	radio_write8(AX5043_REG_PKTADDR1, axradio_localaddr.addr[1]);
	radio_write8(AX5043_REG_PKTADDR2, axradio_localaddr.addr[2]);
	radio_write8(AX5043_REG_PKTADDR3, axradio_localaddr.addr[3]);

	radio_write8(AX5043_REG_PKTADDRMASK0, axradio_localaddr.mask[0]);
	radio_write8(AX5043_REG_PKTADDRMASK1, axradio_localaddr.mask[1]);
	radio_write8(AX5043_REG_PKTADDRMASK2, axradio_localaddr.mask[2]);
	radio_write8(AX5043_REG_PKTADDRMASK3, axradio_localaddr.mask[3]);

	if (axradio_phy_pn9 && axradio_framing_addrlen) {
		uint16_t __autodata pn = 0x1ff;
		uint8_t __autodata inv = -(radio_read8(AX5043_REG_ENCODING) & 0x01);
		if (axradio_framing_destaddrpos != 0xff)
			pn = pn9_advance_bits(pn, axradio_framing_destaddrpos << 3);
		radio_write8(AX5043_REG_PKTADDR0, (radio_read8(AX5043_REG_PKTADDR0) ^ (pn ^ inv)));
		pn = pn9_advance_byte(pn);
		radio_write8(AX5043_REG_PKTADDR1, (radio_read8(AX5043_REG_PKTADDR1) ^ (pn ^ inv)));
		pn = pn9_advance_byte(pn);
		radio_write8(AX5043_REG_PKTADDR2, (radio_read8(AX5043_REG_PKTADDR2) ^ (pn ^ inv)));
		pn = pn9_advance_byte(pn);
		radio_write8(AX5043_REG_PKTADDR3, (radio_read8(AX5043_REG_PKTADDR3) ^ (pn ^ inv)));
	}
}
...
#ifdef DBGPKT
	radio_write8(AX5043_REG_PINFUNCPWRAMP, 0x06); // output pwramp
#endif
	radio_write8(AX5043_REG_PKTLENOFFSET, (radio_read8(AX5043_REG_PKTLENOFFSET) + axradio_framing_swcrclen)); // add len offs for software CRC16 (used for both, fixed and variable length packets
	radio_write8(AX5043_REG_PINFUNCIRQ, 0x03); // use as IRQ pin
	radio_write8(AX5043_REG_PKTSTOREFLAGS, (axradio_phy_innerfreqloop ? 0x13 : 0x15)); // store RF offset, RSSI and delimiter timing
	axradio_setaddrregs();
}
...
static void axradio_timer_callback(struct wtimer_desc __xdata *desc)
{
	...
	chanstatecb:
		...
		axradio_cb_channelstate.st.error = AXRADIO_ERR_NOERROR;
		{
			int8_t __autodata r = radio_read8(AX5043_REG_RSSI);
			axradio_cb_channelstate.st.cs.rssi = r - (int16_t)axradio_phy_rssioffset;
			axradio_cb_channelstate.st.cs.busy = r >= axradio_phy_channelbusy;
		}
		axradio_cb_channelstate.st.time.t = axradio_timeanchor.radiotimer;
	...
	transmitcs:
		if (axradio_ack_count)
			--axradio_ack_count;
		wtimer_remove(&axradio_timer);
		if ((int8_t)radio_read8(AX5043_REG_RSSI) < axradio_phy_channelbusy ||
	...
	case AXRADIO_MODE_WOR_ACK_RECEIVE:
		if (axradio_syncstate == syncstate_lbt)
			goto transmitcs;
	transmitack:
		radio_write8(AX5043_REG_FIFOSTAT, 3);
		radio_write8(AX5043_REG_PWRMODE, AX5043_PWRSTATE_FULL_TX);
		while (!(radio_read8(AX5043_REG_POWSTAT) & 0x08)); // wait for modem vdd so writing the FIFO is safe
		ax5043_init_registers_tx();
		radio_read8(AX5043_REG_RADIOEVENTREQ0); // make sure REVRDONE bit is cleared, so it is a reliable indicator that the packet is out
		radio_write8(AX5043_REG_FIFOTHRESH1, 0);
		radio_write8(AX5043_REG_FIFOTHRESH0, 0x80);
	...
	case AXRADIO_MODE_SYNC_ACK_SLAVE:
		...
		{
			uint8_t __autodata rs = radio_read8(AX5043_REG_RADIOSTATE);
			if (!rs)
				break;
...
static void axradio_receive_callback_fwd(struct wtimer_callback __xdata *desc)
{
	...
	if (axradio_phy_pn9 && !AXRADIO_MODE_IS_STREAM_RECEIVE(axradio_mode)) {
		...
		len += axradio_framing_maclen;
		pn9_buffer((__xdata uint8_t *)axradio_cb_receive.st.rx.mac.raw, len, 0x1ff, -(radio_read8(AX5043_REG_ENCODING) & 0x01));
	}
	...
	if (axradio_mode == AXRADIO_MODE_ACK_TRANSMIT ||
		...
		wtimer_remove_callback(&axradio_cb_transmitend.cb);
		axradio_cb_transmitend.st.error = AXRADIO_ERR_NOERROR;
		axradio_cb_transmitend.st.time.t = radio_read24(AX5043_REG_TIMER2);
		wtimer_add_callback(&axradio_cb_transmitend.cb);
	}
	if (axradio_framing_destaddrpos != 0xff)
	...
	if (axradio_mode == AXRADIO_MODE_ACK_RECEIVE ||
		...
		if (axradio_phy_pn9) {
			pn9_buffer(axradio_txbuffer, axradio_txbuffer_len, 0x1ff, -(radio_read8(AX5043_REG_ENCODING) & 0x01));
		}
		radio_write8(AX5043_REG_IRQMASK1, 0x00);
		radio_write8(AX5043_REG_IRQMASK0, 0x00);
		radio_write8(AX5043_REG_PWRMODE, AX5043_PWRSTATE_XTAL_ON);
		...
		while (radio_read8(AX5043_REG_POWSTAT) & 0x08);
		wtimer_remove(&axradio_timer);
  ...
  endcb:
	if (axradio_mode == AXRADIO_MODE_WOR_RECEIVE) {
		...
		case trxstate_rx:
		case trxstate_rxwor:
			radio_write8(AX5043_REG_IRQMASK0, (radio_read8(AX5043_REG_IRQMASK0) | 0x01)); // re-enable FIFO not empty irq
			break;
...
static uint16_t axradio_tunevoltage(void)
{
	...
	r = (((uint16_t)(radio_read8(AX5043_REG_GPADC13VALUE1) & 0x03)) << 8) | radio_read8(AX5043_REG_GPADC13VALUE0);
	r <<= 6;
	do {
		do {
			radio_write8(AX5043_REG_GPADCCTRL, 0x84);
...
static __reentrantb uint8_t axradio_adjustvcoi(uint8_t rng) __reentrant
{
	...
	for (offs = 0; offs != 16; ++offs) {
		...
		if (!((uint8_t)(rng + offs) & 0xC0)) {
			radio_write8(AX5043_REG_PLLVCOI, (0x80 | (rng + offs)));
...
static __reentrantb uint8_t axradio_calvcoi(void) __reentrant
{
	...
	for (i = 0x40; i != 0;) {
		...
		radio_read8(AX5043_REG_PLLRANGINGA); // clear PLL lock loss
		...
		if (curtune < vmin) {
			...
			if (!(0xC0 & (uint8_t)~(radio_read8(AX5043_REG_PLLRANGINGA))))
				r = i | 0x80;
		}
	}
...
uint8_t axradio_init(void)
{
	...
	for (i = 0; i < axradio_phy_nrchannels; ++i) {
		...
		radio_write8(AX5043_REG_IRQMASK1, 0x10); // enable pll autoranging done interrupt
		{
			...
			radio_write8(AX5043_REG_PLLRANGINGA, r); // init ranging process starting from "range"
		}
		for (;;) {
			reenter_critical();
		...
		axradio_trxstate = trxstate_off;
		radio_write8(AX5043_REG_IRQMASK1, 0x00);
		axradio_phy_chanpllrng[i] = (uint8_t)radio_read8(AX5043_REG_PLLRANGINGA);
		exit_critical(crit);
	}
	// VCOI Calibration
	if (axradio_phy_vcocalib) {
		...
		radio_write8(AX5043_REG_FSKDEV1, 0x00);
		radio_write8(AX5043_REG_FSKDEV0, 0x00);
		radio_write8(AX5043_REG_PLLLOOP, (radio_read8(AX5043_REG_PLLLOOP) | 0x04));
		{
		...
		{
			uint8_t __autodata vcoisave = radio_read8(AX5043_REG_PLLVCOI);
			uint8_t j = 2;
			for (i = 0; i < axradio_phy_nrchannels; ++i) {
				axradio_phy_chanvcoi[i] = 0;
				if (axradio_phy_chanpllrng[i] & 0x20)
					continue;
				radio_write8(AX5043_REG_PLLRANGINGA, (axradio_phy_chanpllrng[i] & 0x0F));
				{
					uint32_t __autodata f = axradio_phy_chanfreq[i];
					radio_write8(AX5043_REG_FREQA0, f);
					radio_write8(AX5043_REG_FREQA1, (f >> 8));
	...
	ax5043_init_registers();
	ax5043_set_registers_rx();
	radio_write8(AX5043_REG_PLLRANGINGA, (axradio_phy_chanpllrng[0] & 0x0F));
	{
...
uint8_t axradio_set_mode(uint8_t mode)
{
	...
	case AXRADIO_MODE_STREAM_TRANSMIT_SCRAM_LSB:
		...
		if (axradio_mode == AXRADIO_MODE_STREAM_TRANSMIT_UNENC ||
			...
			radio_write8(AX5043_REG_ENCODING, 0);
		if (axradio_mode == AXRADIO_MODE_STREAM_TRANSMIT_SCRAM ||
		    axradio_mode == AXRADIO_MODE_STREAM_TRANSMIT_SCRAM_LSB)
			radio_write8(AX5043_REG_ENCODING, 4);
		if (axradio_mode == AXRADIO_MODE_STREAM_TRANSMIT_UNENC_LSB ||
		    axradio_mode == AXRADIO_MODE_STREAM_TRANSMIT_SCRAM_LSB)
			radio_write8(AX5043_REG_PKTADDRCFG, (radio_read8(AX5043_REG_PKTADDRCFG) & 0x7F));
		ax5043_init_registers_tx();
		radio_write8(AX5043_REG_FRAMING, 0);
		ax5043_prepare_tx();
	...
	case AXRADIO_MODE_STREAM_RECEIVE_DATAPIN:
		...
		if (axradio_mode == AXRADIO_MODE_STREAM_RECEIVE_UNENC_LSB ||
			...
			radio_write8(AX5043_REG_PKTADDRCFG, (radio_read8(AX5043_REG_PKTADDRCFG) & 0x7F));
		radio_write8(AX5043_REG_FRAMING, 0);
		radio_write8(AX5043_REG_PKTCHUNKSIZE, 8); // 64 byte
		radio_write8(AX5043_REG_RXPARAMSETS, 0x00);
		if (axradio_mode == AXRADIO_MODE_STREAM_RECEIVE_DATAPIN) {
			ax5043_set_registers_rxcont_singleparamset();
			radio_write8(AX5043_REG_PINFUNCDATA, 0x04);
			radio_write8(AX5043_REG_PINFUNCDCLK, 0x04);
		}
		ax5043_receiver_on_continuous();
		goto enablecs;
...
{
	return axradio_phy_chanpllrng[axradio_curchannel] & 0x000F;
}
...
uint8_t axradio_get_pllvcoi(void)
{
    ...
    return radio_read8(AX5043_REG_PLLVCOI);
}
...
uint8_t axradio_transmit(const struct axradio_address __genericaddr *addr, const uint8_t __genericaddr *pkt, uint16_t pktlen)
{
	...
	case AXRADIO_MODE_STREAM_TRANSMIT_SCRAM_LSB:
	{
		uint16_t __autodata fifofree = radio_read16(AX5043_REG_FIFOFREE1); ///
	...
	if (pktlen) {
		...
		do {
			radio_write8(AX5043_REG_FIFODATA, *pkt++);
		} while (--i);
	}
	{
		criticalsection_t crit = enter_critical();
		radio_read8(AX5043_REG_RADIOEVENTREQ0);
		radio_read8(AX5043_REG_IRQREQUEST0);
...
static __reentrantb uint8_t axradio_set_paramsets(uint8_t val) __reentrant
{
	if (!AXRADIO_MODE_IS_STREAM_RECEIVE(axradio_mode))
		return AXRADIO_ERR_NOTSUPPORTED;
	radio_write8(AX5043_REG_RXPARAMSETS, val);
	return AXRADIO_ERR_NOERROR;
}
...
void axradio_calibrate_lposc(void)
{
        ...
        radio_write8(AX5043_REG_LPOSCREF1, (((axradio_fxtal/640)>>8) & 0xFF));
        radio_write8(AX5043_REG_LPOSCREF0, (((axradio_fxtal/640)>>0) & 0xFF));
        radio_write8(AX5043_REG_PWRMODE, AX5043_PWRSTATE_SYNTH_RX);
        radio_write8(AX5043_REG_LPOSCKFILT1, ((axradio_lposckfiltmax >> (8 + 1)) & 0xFF)); // kfiltmax >> 1
        radio_write8(AX5043_REG_LPOSCKFILT0, ((axradio_lposckfiltmax >> 1) & 0xFF));
        axradio_wait_for_xtal();
...
#endif
        radio_write8(AX5043_REG_LPOSCKFILT1, ((axradio_lposckfiltmax >> (8 + 2)) & 0xFF)); // kfiltmax >> 2
        radio_write8(AX5043_REG_LPOSCKFILT0, ((axradio_lposckfiltmax >> 2) & 0xFF));
...
{
    uint8_t modulation = radio_read8(AX5043_REG_MODULATION);
...
{
    return (radio_read8(AX5043_REG_MODCFGA) & 0x03);
}
